...
class WorkerCommand : SuspendingCliktCommand() {
    val secure: Boolean by option().flag()
    val hostname: String by option("-h", "--host", "--hostname").default("localhost")
    val port: Int? by option("-p", "--port").int()
    val jobs: Int by option().int().default(4).validate { require(it > 0) }

    override suspend fun run() {
        val preconfigured = OkHttpClient()
        val client = ArcadeClient(hostname, port, secure, preconfigured)
